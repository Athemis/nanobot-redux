...
from squidbot import __logo__, __version__
from squidbot.config.schema import Config

app = typer.Typer(
    name="squidbot",
    help=f"{__logo__} squidbot - Personal AI Assistant",
...
OPENROUTER_DEFAULT_EXTRA_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/squidbot",
    "X-Title": "squidbot",
}
...
def _init_prompt_session() -> None:
    ...
    history_file = Path.home() / ".squidbot" / "history" / "cli_history"
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} squidbot[/cyan]")
    console.print(body)
    console.print()
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} squidbot v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """squidbot - Personal AI Assistant."""
    pass
...
@app.command()
def onboard():
    """Initialize squidbot configuration and workspace."""
    from squidbot.config.loader import get_config_path, load_config, save_config
    from squidbot.config.schema import Config
    from squidbot.utils.helpers import get_workspace_path
    ...
    console.print(f"\n{__logo__} squidbot is ready!")
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.squidbot/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]squidbot agent -m \"Hello!\"[/cyan]")
    console.print("\n[dim]Want Matrix/Email channels? See: https://github.com/HKUDS/squidbot#-chat-apps[/dim]")
...
I am squidbot, a lightweight AI assistant.
...
def _make_provider(config: Config):
    ...
    if not model.startswith("bedrock/") and not (p and p.api_key):
        ...
        console.print("Set one in ~/.squidbot/config.json under providers section")
        raise typer.Exit(1)
...
def gateway(
    ...
    """Start the squidbot gateway."""
    from squidbot.agent.loop import AgentLoop
    from squidbot.bus.queue import MessageBus
    from squidbot.channels.manager import ChannelManager
    from squidbot.config.loader import get_data_dir, load_config
    from squidbot.cron.service import CronService
    from squidbot.cron.types import CronJob
    from squidbot.heartbeat.service import HeartbeatService
    from squidbot.session.manager import SessionManager

    if verbose:
        import logging
        logging.basicConfig(level=logging.DEBUG)

    console.print(f"{__logo__} Starting squidbot gateway on port {port}...")
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show squidbot runtime logs during chat"),
):
    ...
    from squidbot.agent.loop import AgentLoop
    from squidbot.bus.queue import MessageBus
    from squidbot.config.loader import load_config
    ...
    if logs:
        logger.enable("squidbot")
    else:
        logger.disable("squidbot")
    ...
    def _thinking_ctx():
        ...
        return console.status("[dim]squidbot is thinking...[/dim]", spinner="dots")
...
def channels_status():
    ...
    email_config = email.imap_host if email.imap_host else "[dim]not configured[/dim]"
    table.add_row(
        "Email",
...
def cron_list(
    ...
    """List scheduled jobs."""
    from squidbot.config.loader import get_data_dir
    from squidbot.cron.service import CronService
...
def cron_add(
    ...
    from squidbot.config.loader import get_data_dir
    from squidbot.cron.service import CronService
    from squidbot.cron.types import CronSchedule
...
def status():
    ...
    console.print(f"{__logo__} squidbot Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from squidbot.providers.registry import PROVIDERS
