...
📏 Real-time line count: **4,327 lines** (run `bash core_agent_lines.sh` to verify anytime)
...
- Matrix typing continuity during tool execution: progress updates no longer cancel typing keepalive, so Element clients keep showing "bot is typing" until the final response is sent.
