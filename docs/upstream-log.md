...
| [#612](https://github.com/HKUDS/nanobot/pull/612) (adapted)                                                                                     | Shell tool      | Adds configurable exec `command_wrapper` for [HKUDS/nanobot#576](https://github.com/HKUDS/nanobot/issues/576), implemented as a hardened POSIX variant to keep wrapped execution semantics predictable | medium | 2026-02-17 | `ruff check nanobot/config/schema.py nanobot/agent/tools/shell.py nanobot/agent/loop.py nanobot/agent/subagent.py tests/test_shell_guard.py tests/test_generation_params.py` + `pytest -q tests/test_shell_guard.py tests/test_generation_params.py` |
