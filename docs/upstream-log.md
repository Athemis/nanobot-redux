...
| [#786](https://github.com/HKUDS/nanobot/pull/786) | Custom provider | New `CustomProvider` bypasses LiteLLM for the `custom` endpoint via `openai.AsyncOpenAI` directly — fixes model-prefix mangling for local/self-hosted servers. Cherry-pick `e2a0d63` with one conflict resolved: import order in `_make_provider` sorted per ruff; upstream's improved `is_oauth` guard in API key check adopted. | low | 2026-02-18 | `ruff check nanobot/providers/custom_provider.py nanobot/providers/registry.py nanobot/cli/commands.py` + `pytest -q` |
