...
| [#786](https://github.com/HKUDS/nanobot/pull/786) | Custom provider | New `CustomProvider` class bypasses LiteLLM entirely and uses `openai.AsyncOpenAI` directly — fixes model-prefix mangling for local/self-hosted servers. Breaking change for existing `custom` provider users (routing mechanism swapped). `commands.py`, `registry.py`, and `config/schema.py` have all diverged in the fork, requiring careful manual rebase against the Pydantic alias_generator refactor (#766). No retry logic in the new provider. | When `config/schema.py` area stabilizes and a local OpenAI-compatible server (e.g. Ollama) is available for end-to-end smoke testing |
