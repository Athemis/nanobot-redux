...
| [#866](https://github.com/HKUDS/nanobot/pull/866) partial (`afca027`) | Memory consolidation | Adopt tool-call schema enforcement for consolidation (`save_memory`) instead of free-form JSON parsing, while keeping fork safeguards: snapshot-based `last_consolidated`, `keep_count==0` safe slicing, `prompt_cache_key`, bool return contract, and defensive string coercion for non-string tool args. | medium | 2026-02-21 | `PYTHONPATH=$PWD pytest -q tests/test_consolidate_offset.py tests/test_memory_consolidation_types.py --no-cov` + `ruff check nanobot/agent/loop.py tests/test_consolidate_offset.py tests/test_memory_consolidation_types.py` |
...
| [#866](https://github.com/HKUDS/nanobot/pull/866) partial (`afca027`) | Memory consolidation | Adopt `save_memory` tool-call schema enforcement for consolidation. Kept fork safeguards while integrating: snapshot-based watermark updates, `keep_count==0` safe slicing, `prompt_cache_key`, and defensive string coercion for non-string tool args. | medium | 2026-02-21 | `PYTHONPATH=$PWD pytest -q tests/test_consolidate_offset.py tests/test_memory_consolidation_types.py --no-cov` + `ruff check nanobot/agent/loop.py tests/test_consolidate_offset.py tests/test_memory_consolidation_types.py` |
| [#930](https://github.com/HKUDS/nanobot/pull/930) partial (`ab026c5`) | Agent loop / memory | Adopt loop-slimming refactors: simplify `_register_default_tools`, inline system-message branch into `_process_message`, and delegate consolidation to `MemoryStore.consolidate`. Excluded only non-functional README/docstring churn while preserving fork-specific consolidation guard behavior. | medium | 2026-02-21 | `PYTHONPATH=$PWD pytest -q tests/test_on_progress.py tests/test_consolidate_offset.py tests/test_memory_consolidation_types.py --no-cov` + `ruff check nanobot/agent/loop.py nanobot/agent/memory.py tests/test_consolidate_offset.py tests/test_memory_consolidation_types.py` |
