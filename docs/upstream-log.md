...
| [#455](https://github.com/HKUDS/nanobot/pull/455) | Encoding / JSON | Explicit `encoding="utf-8"` on all `open()` text-mode calls; `ensure_ascii=False` on all `json.dumps()`/`json.dump()` — fixes garbled output for non-ASCII text (German umlauts, CJK, emoji) on non-UTF-8 systems. Applied to `agent/loop.py`, `agent/subagent.py`, `agent/tools/web.py`, `cli/commands.py`, `config/loader.py`, `cron/service.py`, `heartbeat/service.py`, `session/manager.py`. `channels/dingtalk.py` and `channels/whatsapp.py` hunks excluded (channels not in fork). | low | 2026-02-19 | `ruff check nanobot/` + `pytest -q tests/test_session_manager.py tests/test_config_loader_conversion.py tests/test_cron_service.py` |
