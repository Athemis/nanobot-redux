...
| [#786](https://github.com/HKUDS/nanobot/pull/786) | Custom provider | New `CustomProvider` bypasses LiteLLM for the `custom` endpoint via `openai.AsyncOpenAI` directly — fixes model-prefix mangling for local/self-hosted servers. Neither #786 nor the broader LiteLLM replacement (#165) has been merged upstream, so the provider architecture direction is still open. Adopting now would create a major architectural divergence from upstream at the most critical layer, making future upstream adoption significantly harder. Dependency-slimming goal is compelling but better pursued once upstream resolves direction. | When upstream merges #786, #165, or otherwise settles on the provider architecture |
