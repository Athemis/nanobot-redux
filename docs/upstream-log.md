...
| Unicode fix in persisted JSON files                     | Cron / Session | `json.dumps()` defaults to `ensure_ascii=True`, escaping non-ASCII characters (e.g. umlauts) as `\uXXXX` in `workspace/cron/jobs.json` and session files; pass `ensure_ascii=False` for readable UTF-8 output | low    | 2026-02-17 | `ruff check nanobot/cron/service.py nanobot/session/manager.py`                                                                                                                                                                                                                                                                                                    |
