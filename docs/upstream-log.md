...
| [#65](https://github.com/HKUDS/nanobot/pull/65) | Agent / Tools | Tool response offloading: intercepts tool results >2 KB / 1,000 tokens and saves them to `.artifacts/`; adds `read_artifact`/`tail_artifact`/`search_artifact` tools; new `OffloadConfig` schema. The problem (context overflow from large tool outputs) is real, but the implementation is too heavy for redux: +3,169 lines, new `tiktoken` hard dependency, conflicts with the ~4,000-line core target. Additionally, the PR is unmergeable (`dirty` against upstream `main` as of 2026-02-19), has zero reviews, and has been stale for 16+ days despite upstream releasing v0.1.4 in the interim. **Note:** [Issue #707](https://github.com/HKUDS/nanobot/issues/707) (subagent status tracking) incorrectly references this PR as a fix — the PR does not implement any spawn status commands. The fork's spawn feedback loop is already solved independently via message-bus result announcements in `nanobot/agent/subagent.py`. | 2026-02-19 |
