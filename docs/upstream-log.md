...
| [#823](https://github.com/HKUDS/nanobot/pull/823) | Agent loop | Prevents duplicate memory consolidation tasks: adds `_consolidating: set[str]` guard so only one background consolidation task runs per session key; `finally` block cleans up the flag. Cherry-pick `107a380e` with conflict resolved (type annotation `-> None` added to inner function per project style). | low | 2026-02-20 | `pytest -q tests/test_consolidate_offset.py --no-cov` |
