...
| OpenAI prompt caching controls in OpenAIProvider | [#49](https://github.com/Athemis/nanobot-redux/pull/49) | Providers / Config / CLI | Adds opt-in prompt caching for `chat.completions` via `providers.*.promptCachingEnabled` and optional `promptCacheRetention`, wires config through `_make_provider`, and sends deterministic `prompt_cache_key` to improve cache hit rates without changing default behavior | low | 2026-02-19 | `pytest -q tests/test_generation_params.py tests/test_onboard_openrouter_defaults.py tests/test_config_loader_conversion.py` + `ruff check nanobot/providers/openai_provider.py nanobot/cli/commands.py nanobot/config/schema.py tests/test_generation_params.py tests/test_onboard_openrouter_defaults.py tests/test_config_loader_conversion.py` |
