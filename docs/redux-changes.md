...
| Deterministic `skill:name` expansion in cron | — | Agent loop | `skill:name` cron messages were passed as-is to the LLM, which sometimes asked for clarification or read the wrong SKILL.md instead of executing the named skill. `_process_message` now detects the `skill:name [args]` prefix, loads the SKILL.md programmatically, and injects it directly into the LLM prompt. Session history still records the compact `skill:name` form. | low | 2026-02-19 | `pytest tests/test_skill_invocation.py` + `ruff check nanobot/agent/loop.py tests/test_skill_invocation.py` |
