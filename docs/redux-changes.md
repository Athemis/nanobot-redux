...
| `CronPayload` kind `"skill"` — typed skill dispatch for cron | — | Cron / CLI | Introduces `CronPayload(kind="skill", skill="name")` so cron jobs that invoke skills are expressed structurally rather than via a `skill:name` string convention. `on_cron_job` in the gateway resolves the skill at execution time, injects SKILL.md content, and calls `process_direct` with the expanded prompt. CLI gains `--skill` as an alternative to `--message`. Backwards-compatible: existing jobs without a `skill` key still load correctly. | low | 2026-02-19 | `pytest tests/test_cron_commands.py tests/test_cron_skill_payload.py` + `ruff check nanobot/cron/types.py nanobot/cron/service.py nanobot/cli/commands.py` |
