...
import squidbot.channels.matrix as matrix_module
from squidbot.bus.events import OutboundMessage
from squidbot.bus.queue import MessageBus
from squidbot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from squidbot.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("squidbot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "squidbot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("squidbot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "squidbot.channels.matrix.asyncio.create_task", _fake_create_task
    )
